...
export const USER_PROMPTS: UserPrompts = {
  ...
  INTRO: ({
    ...
    return {
      ...
      content: `${songTitle.length > 0 ? `SONG TITLE: ${songTitle}` : ""}
     ${songDescription.length > 0 ? `SONG DESCRIPTION: ${songDescription}` : ""}
      As an innovative songwriter, create an engaging intro of ${lineLimit} lines that sets the tone for the entire song. Each line should follow the prescribed meter, where 0 is an unstressed syllable and 1 is stressed. ${meterString} Craft an opening that immediately captures the listener's attention and introduces the song's theme or mood. Use vivid imagery, intriguing questions, or a powerful statement to draw the audience in. The intro should seamlessly lead into the first verse or chorus, creating anticipation for what's to come.${
        restOfSong.length > 1
          ? ` Here is the rest of the song for context: ${restOfSong
              .map((component) => {
                return [component.component, ...component.lyrics];
  ...
  OUTRO: ({
    ...
    return {
      ...
      content: `${songTitle.length > 0 ? `SONG TITLE: ${songTitle}` : ""}
     ${songDescription.length > 0 ? `SONG DESCRIPTION: ${songDescription}` : ""}
      As a seasoned songwriter, compose a memorable outro of ${lineLimit} lines that provides a satisfying conclusion to the song. Each line should follow the prescribed meter, where 0 is an unstressed syllable and 1 is stressed. ${meterString} Create an ending that reinforces the song's main message, resolves any tension, or leaves a lasting impression on the listener. Consider using repetition, a final powerful statement, or a twist on earlier lyrics to bring the song full circle. The outro should feel like a natural and impactful way to close the song.${
        restOfSong.length > 1
          ? ` Here is the rest of the song for context: ${restOfSong
              .map((component) => {
                return [component.component, ...component.lyrics];
